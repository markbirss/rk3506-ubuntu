#!/bin/bash
# Audio Configuration Verification Script
# Run this on the board after flashing new device tree
...
echo "  Luckfox Lyra Zero W - SAI1 Diagnostic Tool"
echo "==================================================================="
echo ""
...
check_value() {
    ...
    local path="/sys/firmware/devicetree/base/sai@ff310000/$prop"
    if [ -f "$path" ]; then
        local value=$(cat "$path" | tr -d '\0')
        if [ "$value" = "$expected" ]; then
            echo -e "${GREEN}✓${NC} $prop = $expected"
            return 0
        else
            echo -e "${YELLOW}⚠${NC} $prop = $value (expected: $expected)"
            return 1
        fi
    else
...
# Check that sound-dai does NOT exist (should be fixed)
...
if [ -f /sys/firmware/devicetree/base/sai@ff310000/pinctrl-0 ]; then
    ...
    if [ $pin_count -eq 4 ]; then
        ...
        echo "   Expected: MCLK, SCLK, LRCK, SDO0"
    else
...
if groups | grep -q "audio"; then
    ...
    echo -e "${YELLOW}⚠${NC} User NOT in audio group (run: sudo usermod -aG audio $USER)"
    ((errors++))
fi
echo ""

# 7. Test Audio Device Access
echo "7. Audio Device Access Test"
echo "-------------------------------------------------------------------"
...
# 8. Quick Speaker Test
echo "8. Quick Audio Playback Test (1 second)"
echo "-------------------------------------------------------------------"
echo "Testing 1kHz sine wave on hw:0,0..."
if timeout 3 speaker-test -D hw:0,0 -t sine -f 1000 -c 2 -l 1 >/dev/null 2>&1; then
    ...
    if [ $exit_code -eq 124 ]; then
        ...
        echo "   This means device tree is still incorrect."
        ((errors++))
    else
...
if [ $errors -eq 0 ]; then
    ...
    echo "See: /home/nadlech/privat/luckfox/rk3506-ubuntu/audio-config/DIAGNOSIS.md"
    echo ""
    exit 1
fi
