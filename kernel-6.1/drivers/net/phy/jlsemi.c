...
static int jl1xxx_config_init(struct phy_device *phydev)
{
	...
	phy_write(phydev, 0x1f, JL1XXX_PAGE7);
	rdata = jlsemi_read_paged(phydev,JL1XXX_PAGE7,JL1XXX_LED_REG);	// read register 19 config
	rdata = rdata | (1 << 3); // enable led custom function
	phy_write(phydev, 0x13, rdata);
	
	phy_write(phydev, 0x1f, JL1XXX_PAGE129);	//page_sel -> 129
	rdata = jlsemi_read_paged(phydev,JL1XXX_PAGE129,JL1XXX_REG24);	// read led custom register
	
	rdata = rdata | (1 << 13); // LED1 enable 10 Link
	rdata = rdata | (1 << 12); // LED1 enable 100 Link
	rdata = rdata & ~(1 << 11); // LED1 disable 10 Activity
	rdata = rdata & ~(1 << 10); // LED1 disable 100 Activity

	rdata = rdata & ~(1 << 5);// LED0 disable 10 Link
	rdata = rdata & ~(1 << 4); // LED0 disable 100 Link
	rdata = rdata | (1 << 3); // LED0 enable 10 Activity
	rdata = rdata | (1 << 2); // LED0 enable 100 Activity
	
	phy_write(phydev, 0x18, rdata);		// read led custom register
	return 0;
}
